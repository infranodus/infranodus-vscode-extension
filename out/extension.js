    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        ...
        if (apiKey) {
          ...
          await vscode.workspace.getConfiguration("infranodus-graph-view").update("apiKey", apiKey, vscode.ConfigurationTarget.Global);
...
var InfraNodusViewProvider = class _InfraNodusViewProvider {
  ...
  resolveWebviewView(webviewView, context, _token) {
    ...
    this.topicsSubject.subscribe((data) => {
      ...
      const topicNames = data.entriesAndGraphOfContext?.graph?.graphologyGraph?.attributes?.top_clusters.map(
        (topic) => {
          if (topic.aiName) {
      ...
      switch (message.command) {
        ...
        case "processExternalAction":
          ...
          if (actionMessage == "context" || actionMessage == "context_gap") {
            ...
            if (selectedWords.length > 0) {
              ...
              if (actionMessage == "context_gap") {
                ...
                const all3 = this.getAllStatementsOfTopics({
                  statements,
                  selectedTopics: selectedClusters
                });
                contextText = all3.join("\n\n");
          ...
          if (selectedClusters.length > 0 && selectedWords.length == 0) {
            statementsToUse = actionMessage == "summarize" || actionMessage == "graph summary" || actionMessage == "context" ? this.getAllStatementsOfTopics({
              statements,
              selectedTopics: selectedClusters
...
${graphContent}`;
  ...
  async getApiKey() {
    ...
    return configuredApiKey || await this._context.secrets.get("infranodus-api-key");
  }
